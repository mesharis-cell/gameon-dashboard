...
interface ActivationDetailSheetProps {
...
export function ActivationDetailSheet({
  ...
  onEdit,
  onDuplicate,
}: ActivationDetailSheetProps) {
  if (!activation) return null;

  const brandName = brand?.name || "Unknown";
  const brandLogo = brand?.logoUrl;
  const brandColor = brand?.primaryColor || "#6366f1"; // Default to indigo if no color
        ...
        <div className="flex flex-col gap-y-6 mt-2">
          ...
          <div className="px-4">
            <h2 className="text-2xl font-bold mb-2">
              {brandName}
              <br />
              <p className="text-lg">@ {activation.name}</p>
            </h2>
          ...
          <div className="flex gap-2 ">
            ...
            <Can permission="activation:manage:all">
              ...
              <Button className="flex-1" variant="default" onClick={onEdit}>
